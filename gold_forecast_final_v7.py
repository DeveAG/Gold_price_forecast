...
st.set_page_config(page_title="Gold Forecast (v7)", layout="wide")
...
def train_prophet(df, column, future_days, status_area):
    ...
    df_prophet = df[[column]].copy()
    df_prophet = df_prophet.reset_index().rename(columns={"Date": "ds", column: "y"})
    df_prophet['ds'] = pd.to_datetime(df_prophet['ds'])
    model = Prophet(daily_seasonality=True)
    model.fit(df_prophet)
    future = model.make_future_dataframe(periods=future_days)
    forecast = model.predict(future)
    forecast_result = forecast[['ds', 'yhat']].set_index('ds').iloc[-future_days:]
...
if st.button("Run Forecast"):
    ...
    # Show daily OHLC table
    st.subheader("📅 Daily Gold OHLC Table")
    st.dataframe(df.tail(30).round(2), use_container_width=True)

# Show intraday (1-hour) chart table
st.subheader("⏱️ 1-Hour Chart Table (Past 5 Days)")
intraday_df = get_intraday_data()
st.dataframe(intraday_df.tail(48).round(2), use_container_width=True)
