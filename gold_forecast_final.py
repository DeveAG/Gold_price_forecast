...
def train_sarimax(df, column, future_days):
...
st.set_page_config(page_title="Gold Forecast Final", layout="wide")
st.title("📈 Final Gold Forecast App (OHLC + Telegram)")
...
if st.button("Run Forecast"):
    with st.spinner("Fetching data and forecasting..."):
        ...
        st.success(f"Latest price used: {last_date}")

        forecasts = {}
        for col in ['Open', 'High', 'Low', 'Close']:
            forecast = train_sarimax(df, col, future_days)
        ...
        if telegram_token and chat_id:
            message = f"Gold Forecast Complete ✅\nModel: SARIMAX\nDays: {future_days}\nLast Price Date: {last_date}"
