...
st.title("📱 Gold Price Forecasting with Live Learning")
...
live_learning = st.checkbox("🔁 Enable daily live retraining", value=True)
...
if st.button("📈 Run Forecast"):
    with st.spinner("Running forecasting pipeline..."):
        ...
        if live_learning:
            # no saved model; live retrain every time
            ...
            st.warning("Live retraining is required each time currently. Persistent models not implemented yet.")
        ...
        st.download_button("📥 Download Forecast CSV", open(forecast_file, "rb"), forecast_file)
